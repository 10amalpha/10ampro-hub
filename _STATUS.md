...
- **PortfolioEmbed.jsx** — Client-only component (dynamic import, SSR disabled). Multi-wallet support: detects Phantom, Backpack, xNFT via native window providers. Silent auto-connect tries all providers, falls back to wallet chooser buttons. Inline activation flow (code input inside hub, no redirect). Mobile deep link opens Phantom browser. Checks activation via shared Supabase, fetches holdings cross-origin from portafoliotracker.vercel.app APIs (CORS middleware on portafoliotracker). Zero new npm deps. Five states: no_wallet → disconnected → not_activated → loading → ready. Sits between macro bar and calendar.
