// ============================================================
// /api/insights — AI-generated editorial insights
// Fetches 10am.pro RSS + market context → Anthropic API → 6 insights
// ============================================================
...
// ─── Call Anthropic API ─────────────────────────────────────
async function generateInsights(articles, market) {
...
- Bessent "3-3-3": déficit a 3% GDP, 3% crecimiento, 3M bpd oil. Este framework guía la política económica de EE.UU.
- El mercado se mueve por narrativa, no solo por fundamentales.
- Tesis secular de 10AMPRO: macro regime shift — liquidez global expandiendo, AI como catalizador secular, cripto como infraestructura financiera.

WATCHLIST CLAVE DE 10AMPRO:
...
3. AL MENOS 2 de los 6 insights DEBEN incluir un link a un artículo real del catálogo RSS. Usa URLs exactas — NO inventes URLs.
4. Los links deben ser contextualmente relevantes. Si hablas de liquidez, linkea el artículo de liquidez.
5. El label del link: descriptivo y atractivo. "Cómo leemos la liquidez →" no "Leer más →".
6. No repitas el mismo tag dos veces.
7. Responde SOLO con un JSON array válido. Sin markdown, sin backticks, sin explicaciones.

FORMATO:
[
  { "tag": "MACRO", "color": "#60a5fa", "text": "Tu insight aquí.", "link": { "label": "Texto del link →", "url": "https://www.10am.pro/p/..." } },
  { "tag": "BTC", "color": "#f59e0b", "text": "Insight sin link.", "link": null }
]`;
...
Genera los 6 insights editoriales. Al menos 2 con link a artículos del catálogo. Solo JSON.`;
...
const FALLBACK = [
  { tag: 'MACRO', color: '#60a5fa', text: 'Mercados en modo wait-and-see. La atención está en datos de empleo y las minutas de la Fed. Si el mercado laboral muestra debilidad, la tesis de recortes cobra fuerza.', link: { label: 'El framework macro de 10AMPRO →', url: 'https://www.10am.pro' } },
  { tag: 'RISK', color: '#facc15', text: 'VIX es el termómetro. Debajo de 20 es complacencia, arriba de 30 es miedo. Entre 20-25 es indecisión.', link: null },
...
export async function GET() {
  try {
    ...
    console.log(`Insights API: ${articles.length} RSS articles, ${Object.keys(market).length} market keys`);
