...
export async function GET(req, { params }) {
  const TOKEN = getHub(params.hub); if (!TOKEN) return new Response('not found', { status: 404 });
  const origin = new URL(req.url).origin;
  let m = null, px = null;
  try { const r = await fetch(`${origin}/api/hub/${TOKEN.slug}/network`, { cache: 'no-store' }); const j = await r.json(); m = Object.values(j.metrics || {}).find((x) => x.primary) || Object.values(j.metrics || {})[0] || null; } catch {}
  try { const r = await fetch(`${origin}/api/hub/${TOKEN.slug}/market`, { cache: 'no-store' }); px = await r.json(); } catch {}
